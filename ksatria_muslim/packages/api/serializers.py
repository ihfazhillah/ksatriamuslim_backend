...
class PackageLogUsageSerializer(serializers.Serializer):
    ...
    finished_at = serializers.DateTimeField(required=False, format="%Y%m%d%H%M", input_formats=["%Y%m%d%H%M"])
    started_at = serializers.DateTimeField(required=False, format="%Y%m%d%H%M", input_formats=["%Y%m%d%H%M"])
